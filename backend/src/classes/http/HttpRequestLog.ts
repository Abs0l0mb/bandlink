...
import { HttpRequest, HttpResponse, Log } from '@src/classes';
...
export class HttpRequestLog {
    ...
    */
    private log() {
        ...
        console.log(`${new Date().toJSON()} ${this.getHttpRequestLog()} ${this.getHttpResponseLog()}`);
    }
    ...
    */
    private getHttpRequestLog() : string {
        ...
        if (software)
            return `${remoteSocket} ${software} ${method} ${url} ${parameters}`;
        else
            return `${remoteSocket} ${method} ${url} ${parameters}`;
    }
