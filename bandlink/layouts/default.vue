<template>
    <section @triedLogIn="verifySession()"
        class="border-b border-gray-200 relative w-full px-4 text-gray-700 bg-white body-font tails-selected-element"
        data-tails-scripts="//unpkg.com/alpinejs">
        <div class="container flex flex-col flex-wrap items-center justify-between py-5 mx-auto md:flex-row max-w-7xl">
            <NuxtLink to="/" class="relative z-10 flex items-center w-auto text-xl leading-none text-black select-none">
                <img class='w-[3w] h-[3vw] mr-2' src='../public/favicon.ico'>Bandlink
            </NuxtLink>
            <nav
                class="top-0 left-0 z-0 flex items-center justify-center w-full h-full py-5 -ml-0 space-x-10 text-base text-black md:-ml-5 md:py-0 md:absolute">
                <NuxtLink to="/bands"
                    class="relative font-medium duration-150 hover:text-gray-900 opacity-60 hover:opacity-100">
            ...
            <div v-if="refData.session" class="relative z-10 inline-flex items-center space-x-3 md:ml-5 lg:justify-end">
                <NuxtLink to="/account"
                    class="inline-flex items-center justify-center px-4 py-2 text-base font-medium leading-6 text-black opacity-60 hover:opacity-100 whitespace-no-wrap bg-white border border-gray-200 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:shadow-none"
    ...
    <div class="text-black duration-800 bg-gray-100 min-h-screen">
        <slot />
    </div>
</template>
